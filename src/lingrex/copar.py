...
class CoPaR(Alignments):
    ...
    def _check(self):
        ...
        for idx, struc, alm in self.iter_rows(self._structure, self._alignment):
            ...
            if not len(self[idx, self._structure]) == len([x for x in 
                self[idx, self._alignment] if x != '-']):
                print(idx, self[idx, self._structure], '|', self[idx,
    ...
    def get_sites(self):
        ...
        errors = self._check()
        if errors:
            raise ValueError("found {0} problems in the data".format(
                len(errors)))
        ...
        for cogid, msa in pb(
            ...
            if len(_taxa) >= self.minrefs:
                if self._mode == 'fuzzy':
                    ...
                    _strucs = [self[idx, self._structure] for idx in \
                            _wlid]
                ...
                while sorted_clusters:
                    ...
                    for (next_pos, next_cluster), next_vals in remaining_clusters:
                        match, mism = compatible_columns(
                                ...
                                missing=self.missing, gap=self.gap)
            ...
            if self._mode == 'fuzzy':
                for indices in [idx for idx in self.etd[self.ref][cogid] if
                    ...
                    for widx in indices:
                        ...
                        alms[cog_pos] = new_alm
                        self[widx, self._alignment] = self._str_type(' + '.join([
